...
namespace PEPCO_Propulsion
{
    public class VectorThrustEnginesControls
    {
        ...
        static bool CustomVisibleCondition(IMyTerminalBlock b)
        {
            ...
            return b?.GameLogic?.GetAs<PEPCOWaterThrustBlock>() != null;
        }
        ...
        static void CreateControls()
        {
            ...
            //Toggle button for reversing thrust vectoring on/off
            {
                ...
                c.Getter = (b) => b?.GameLogic?.GetAs<PEPCOWaterThrustBlock>()?.VectorThrustReverse_Toggle ?? true;
                c.Setter = (b, v) =>
                {
                    var logic = b?.GameLogic?.GetAs<PEPCOWaterThrustBlock>();
                    if (logic != null)
        ...
        static void CreateActions(IMyModContext context)
        {
            // yes, there's only one type of action
            {
                ...
                a.Action = (b) => {
                    var logic = b?.GameLogic?.GetAs<PEPCOWaterThrustBlock>();
                    logic.VectorThrustReverse_Toggle = !logic.VectorThrustReverse_Toggle;
                };
                ...
                {
                    var logic = b?.GameLogic?.GetAs<PEPCOWaterThrustBlock>();
                    if (logic.VectorThrustReverse_Toggle)
                    {
                        sb.Append($"On");
                    }
                    else
