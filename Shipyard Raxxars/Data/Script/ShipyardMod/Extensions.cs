...
namespace ShipyardMod.Utility
{
    public static class Extensions
    {
        ...
        public static bool IsInVoxels(this IMyCubeGrid grid)
        {
            ...
            Utilities.InvokeBlocking(()=>entities = MyAPIGateway.Entities.GetTopMostEntitiesInBox(ref box));
            ...
            {
                IMyVoxelBase voxel = (IMyVoxelBase)vox;
                foreach (var block in blocks)
                {
                    BoundingBoxD blockBox;
                    block.GetWorldBoundingBox(out blockBox);
                    var cubeSize = block.CubeGrid.GridSize;
                    BoundingBoxD localAAABB = new BoundingBoxD(cubeSize * ((Vector3D)block.m - 0.5), cubeSize * ((Vector3D)block.Max + 0.5));
